...
interactions_df.registerTempTable("awards")

complaints = sqlContext.sql("""
    SELECT complaint_type, incident_zip, count(*) total FROM awards WHERE length(incident_zip) = 5 and incident_zip like '1%' group by complaint_type, incident_zip
""")
...
complaints.write.format("org.apache.spark.sql.cassandra").options(table ="awards", keyspace = "playground").save(mode='append')
