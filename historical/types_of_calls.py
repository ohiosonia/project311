...
interactions_df.registerTempTable("types_of_calls")

complaints = sqlContext.sql("""
    SELECT complaint_type, incident_zip, count(*) total FROM types_of_calls WHERE length(incident_zip) = 5 and incident_zip like '1%' group by complaint_type, incident_zip
""")
...
complaints.write.format("org.apache.spark.sql.cassandra").options(table ="types_of_calls", keyspace = "playground").save(mode='append')
